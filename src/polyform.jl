...
to_poly!(::Dict, ::Dict, expr, ::Bool) = MA.operate!(+, zeropoly(), expr)
function to_poly!(poly_to_bs::Dict, bs_to_poly::Dict, expr::BasicSymbolic{T}, recurse::Bool = true)::Union{PolyVarT, PolynomialT} where {T}
    type = symtype(expr)
    @match expr begin
